...
namespace HPSocketCS
{
    public class UdpServer : ConnectionExtra
    {
        ...
        /// <summary>
        /// 读取或设置心跳检查间隔（毫秒，0 不发送心跳包，如果超过若干次 [默认：WinXP 5 次, Win7 10 次] 检测不到心跳确认包则认为已断线）
        /// </summary>
        ...
        // if (obj != null) { // 设置附加数据 ptr = Marshal.AllocHGlobal(Marshal.SizeOf(obj));
        ...
        // IntPtr ptr = IntPtr.Zero; // 释放附加数据 if (Sdk.HP_Server_GetConnectionExtra(pServer, connId,
        // ref ptr) && ptr != IntPtr.Zero) { Marshal.FreeHGlobal(ptr); ptr = IntPtr.Zero; }
