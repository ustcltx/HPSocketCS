...
{
    /// <summary>
    /// SSL 工作模式 描述：标识 SSL 的工作模式，客户端模式或服务端模式
    /// </summary>
    ...
    /// <summary>
    /// 名称：SSL 验证模式 描述：SSL 验证模式选项，SSL_VM_PEER 可以和后面两个选项组合一起
    /// </summary>
    ...
    public class SSLSdk
    {
        ...
        /// <summary>
        /// 名称：SNI 服务名称回调函数 描述：根据服务器名称选择 SSL 证书 返回值： 0 -- 成功，使用默认 SSL 证书 正数 -- 成功，使用返回值对应的 SNI 主机证书
        /// 负数 -- 失败，中断 SSL 握手
        /// </summary>
        ...
        /// <summary>
        /// 清理 SSL 全局运行环境 描述：清理 SSL 全局运行环境，回收 SSL 相关内存 1、应用程序退出时会自动调用调用本方法 2、当要重新设置 SSL 全局环境参数时，需要先调用本方法清理原先的环境参数
        /// </summary>
        [DllImport(HPSOCKET_SSL_DLL_PATH)]
        public static extern void HP_SSL_Cleanup();

        /// <summary>
        /// 名称：初始化 SSL 全局环境参数 描述：SSL 全局环境参数必须在 SSL 通信组件启动前完成初始化，否则启动失败
        /// </summary>
        ...
        /// <summary>
        /// 清理线程局部环境 SSL 资源 描述：清理 SSL 全局运行环境，回收 SSL 相关内存 任何一个操作 SSL 的线程，通信结束时都需要清理线程局部环境 SSL 资源
        /// 1、主线程和 HP-Socket 工作线程在通信结束时会自动清理线程局部环境 SSL 资源。因此，一般情况下不必手工调用本方法 2、特殊情况下，当自定义线程参与
        /// HP-Socket 通信操作并检查到 SSL 内存泄漏时，需在每次通信结束时自定义线程调用本方法
        /// </summary>
        [DllImport(HPSOCKET_SSL_DLL_PATH)]
