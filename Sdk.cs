...
namespace HPSocketCS
{
    ...
    /// <summary>
    /// 名称：操作结果代码 描述：组件 Start() / Stop() 方法执行失败时，可通过 GetLastError() 获取错误代码
    /// </summary>
...
public class Sdk
{
    static Sdk()
    {
        var myPath = new Uri(typeof(Sdk).Assembly.CodeBase).LocalPath;
        var myFolder = System.IO.Path.GetDirectoryName(myPath);
        var subfloder = Environment.Is64BitProcess ? "x64" : "x86";
        LoadLibrary(System.IO.Path.Combine(myFolder, subfloder, HPSOCKET_DLL_PATH));
    }
...
#if WIN64
    public const string HPSOCKET_DLL_PATH = "x64/HPSocket4C_U.dll";
#else
    ...
    /// <summary>
    /// 名称：启动通信组件 描述：启动服务端通信组件，启动完成后可开始接收客户端连接并收发数据
    /// </summary>
    /// <param name="pServer"></param>
    ...
    /// <summary>
    /// 发送多组数据 向指定连接发送多组数据 TCP - 顺序发送所有数据包 UDP - 把所有数据包组合成一个数据包发送（数据包的总长度不能大于设置的 UDP 包最大长度）
    /// </summary>
    /// <param name="pServer"></param>
    /// <param name="connId">连接 ID</param>
    ...
    /// <summary>
    /// 设置连接的附加数据 是否为连接绑定附加数据或者绑定什么样的数据，均由应用程序只身决定
    /// </summary>
    /// <param name="pServer"></param>
    /// <param name="connId">连接 ID</param>
    ...
    /// <summary>
    /// 获取连接的附加数据 是否为连接绑定附加数据或者绑定什么样的数据，均由应用程序只身决定
    /// </summary>
    /// <param name="pServer"></param>
    /// <param name="connId">连接 ID</param>
    ...
    /// <param name="lpszBindAddress">
    /// 绑定地址（默认：nullptr，TcpClient/UdpClient -&gt; 不执行绑定操作，UdpCast 绑定 -&gt; 0.0.0.0）
    /// </param>
    /// <returns>失败，可通过 HP_Client_GetLastError() 获取错误代码</returns>
    [DllImport(HPSOCKET_DLL_PATH, CharSet = CharSet.Unicode)]
    ...
    /// <summary>
    /// 发送多组数据 向服务端发送多组数据 TCP - 顺序发送所有数据包 UDP - 把所有数据包组合成一个数据包发送（数据包的总长度不能大于设置的 UDP 包最大长度）
    /// </summary>
    /// <param name="pClient"></param>
    /// <param name="pBuffers">发送缓冲区数组</param>
    /// <param name="iCount">发送缓冲区数目</param>
    ...
    /// <param name="usPort">服务端端口</param>
    /// <param name="bAsyncConnect">是否采用异步 Connnect</param>
    /// <param name="lpszBindAddress"></param>
    /// <returns>失败，可通过 HP_UdpCast_GetLastError() 获取错误代码</returns>
    [DllImport(HPSOCKET_DLL_PATH, CharSet = CharSet.Unicode)]
    public static extern bool HP_UdpCast_Start(IntPtr pCast, string lpszRemoteAddress, ushort usPort, bool bAsyncConnect, string lpszBindAddress);
    ...
    /// <returns>失败，可通过 HP_UdpCast_GetLastError() 获取错误代码</returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    public static extern bool HP_UdpCast_Stop(IntPtr pCast);

    /// <summary>
    /// 发送数据 用户通过该方法向指定连接发送数据
    /// </summary>
    /// <param name="pAgent"></param>
    /// <param name="connId">连接 ID</param>
    /// <param name="pBuffer">发送数据缓冲区</param>
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH, CharSet = CharSet.Ansi, SetLastError = true)]
    public static extern bool HP_UdpCast_Send(IntPtr pCast, byte[] pBuffer, int length);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH, SetLastError = true)]
    public static extern bool HP_UdpCast_Send(IntPtr pCast, IntPtr pBuffer, int length);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH, CharSet = CharSet.Ansi, SetLastError = true)]
    public static extern bool HP_UdpCast_SendPart(IntPtr pCast, byte[] pBuffer, int length, int iOffset);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH, SetLastError = true)]
    public static extern bool HP_UdpCast_SendPart(IntPtr pCast, IntPtr pBuffer, int length, int iOffset);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    public static extern bool HP_UdpCast_HasStarted(IntPtr pCast);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    public static extern IntPtr HP_UdpCast_GetConnectionID(IntPtr pCast);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    public static extern bool HP_UdpCast_GetRemoteAddress(IntPtr pCast, [MarshalAs(UnmanagedType.LPWStr)] StringBuilder lpszAddress, ref int piAddressLen, ref ushort pusPort);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    public static extern SocketError HP_UdpCast_GetLastError(IntPtr pCast);
    ...
    /// <returns></returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    public static extern IntPtr HP_UdpCast_GetLastErrorDesc(IntPtr pCast);

    /**************************************************************************/
    /***************************** Agent 操作方法 *****************************/

    /// <summary>
    /// 启动通信组件 启动通信代理组件，启动完成后可开始连接远程服务器
    /// </summary>
    /// <param name="pAgent"></param>
    /// <param name="pszBindAddress">监听地址</param>
    /// <param name="bAsyncConnect">是否采用异步 Connect</param>
    ...
    /// <summary>
    /// 关闭通信组件 关闭通信组件，关闭完成后断开所有连接并释放所有资源
    /// </summary>
    /// <param name="pAgent"></param>
    /// <returns>-- 失败，可通过 GetLastError() 获取错误代码</returns>
    [DllImport(HPSOCKET_DLL_PATH)]
    ...
    /// <summary>
    /// 连接服务器 连接服务器，连接成功后 IAgentListener 会接收到 OnConnect() 事件
    /// </summary>
    /// <param name="pAgent"></param>
    ...
    /// <summary>
    /// 窥探数据（不会移除缓冲区数据） 描述：用户通过该方法从 Socket 组件中窥探数据
    /// </summary>
    /// <param name="pServer"></param>
    /// <param name="connId">连接 ID</param>
    ...
    /// <summary>
    /// 名称：窥探数据（不会移除缓冲区数据） 描述：用户通过该方法从 Socket 组件中窥探数据
    /// </summary>
    /// <param name="pClient"></param>
    /// <param name="connId">连接 ID</param>
    /// <param name="pBuffer">数据抓取缓冲区</param>
